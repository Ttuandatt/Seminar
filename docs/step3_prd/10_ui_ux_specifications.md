...
## 11. Delta Screens v3.2

### S31: Shop Owner Analytics Detail
- Route: `/owner/analytics`
- Mục tiêu: xem tổng lượt xem, lượt nghe, biểu đồ theo ngày, top POIs.
- Components: KPI cards, line chart, top POI table, period filter (7/30/90d).
- Interaction: đổi period -> refetch data, click POI -> mở chi tiết POI.

### S32: Merchant Management (Admin)
- Route: `/admin/merchants`, `/admin/merchants/new`, `/admin/merchants/:id/edit`
- Components: merchant table, search/filter, form create/edit.
- Interaction: CRUD đầy đủ, confirm khi xóa.

### S33: Tourist Custom Tour Builder
- Route: `/tour/create`, `/tour/edit/:id`
- Components: POI selector list, selected stops list, reorder control, save action.
- Interaction: chọn điểm -> thêm vào tour -> sắp xếp thứ tự -> lưu.

### S34: Language Selection Expanded
- Route: `/language`
- Mục tiêu: hỗ trợ danh sách 11 ngôn ngữ.
- Components: searchable language list, current-language indicator, confirm switch.

### S35: Onboarding + Device Check Combined Entry
- Route: `/onboarding`, `/device-check`
- Luồng: first open -> onboarding -> device capability check (GPS + Internet) -> tabs.
- Edge cases: denied GPS -> hướng dẫn mở settings; không có mạng -> retry loop.
