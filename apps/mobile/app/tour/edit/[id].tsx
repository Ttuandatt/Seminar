...
export default function EditTourScreen() {
    ...
    const loadTour = async () => {
        try {
            if (typeof id === 'string') {
                const tour = await touristService.getMyTourDetail(id);
                setName(tour.nameVi || '');
                setDescription(tour.descriptionVi || '');
                if (tour.tourPois) {
                    setSelectedPois(tour.tourPois.map((tp: any) => tp.poi));
                }
            }
        } catch (e) {
    ...
    const handleSubmit = async () => {
        ...
        setSubmitting(true);
        try {
            await touristService.updateMyTour(id as string, {
                name: name.trim(),
            ...
            Alert.alert(t('common.error'), t('customTour.updateError'));
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <View style={styles.container}>
            <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.scrollContent}>
                <Text style={styles.headerTitle}>{t('customTour.editTitle')}</Text>
                ...
                >
                    {submitting ? (
                        ...
                        <Text style={styles.submitButtonText}>
                            {t('customTour.save')} ({selectedPois.length} {t('tours.destinations')})
                        </Text>
                    )}
                </TouchableOpacity>
            </View>
